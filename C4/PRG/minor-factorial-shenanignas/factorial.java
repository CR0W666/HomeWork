...
public class factorial {
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.printf("\nresult:\nNormal: %s\nRecursive: %s\nstream: %s\ncustom( O(n/2)? ): %s", factOfInput, recursiveFact, streamFact, customFact);
    ...
    public static long input(Scanner sc, String msg) {
        ...
        try {
            input = sc.nextLong();
            if (input > 20 || input < 0) throw new InputMismatchException(input + " is out of range. Max is 20 and min is 0");
    ...
    public static long customFact(long factorialOf) {
        if(factorialOf == 0) return 1;
        if(factorialOf%2 != 0) return customFact(factorialOf-1)*factorialOf;
    ...
    public static long customFactFirstAttempt(long factorialOf) {
        ...
        long[] sequence = new long[(int) (Math.ceil(factorialOf/2))];
        sequence[0] = factorialOf;
        long temp = factorialOf-2;
        for(int i = 1; i < Math.ceil(factorialOf/2); i++) {
    ...
    public static long[] trial(int numOfRepetitions, long factorialOf, long format) {
        ...
        System.out.printf(
            "\n------------------\ntrial:\n\nnano/%s:\nnormal: %s\nrecur: %s\nstream: %s\ncustom: %s\ncustom old: %s", 
            format, trialResults[0], trialResults[1], trialResults[2], trialResults[3], trialResults[4]);
        //System.out.printf("\nnano:\nnormal: %s\nrecur: %s\nstream: %s\ncustom: %s", trialResults[0], trialResults[1], trialResults[2], trialResults[3]);
