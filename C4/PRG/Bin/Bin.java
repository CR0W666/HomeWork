...
public class Bin {
    ...
    public static long[] init() {
        ...
        long base = input(sc, "please enter a base to convert to (except 10) eq: binary -> 2\ninput: ", false)[0];
        long[] number = input(sc, "Please input a decimal number to be converted into the selected base\ninput: ", true);
        
        System.out.println(spacer);

        sc.close(); //terrible java gc

        return new long[] {number[0], base, number[1]}; //number, base, bool is negative (0 = false)
    }

    public static long[] input(Scanner sc, String msg, boolean negative) {
        System.out.print(msg); //print input message
        //---- SETUP ----
        long[] values = new long[2]; //size of two, first holds the number, second if the num was negative
        //---------------
        ...
        try {
            ...
            if(input<0) values[1] = 1;     // if number WAS negative, store it as 1 (true)
        } catch (Exception e) {
    ...
    public static String properFormat(String number, long base) {
        ...
        for(String num : nums) {
            result += Long.toString(Long.valueOf(num), (int)base);
        }
