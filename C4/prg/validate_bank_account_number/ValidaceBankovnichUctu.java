...
class ValidaceBankovnichUctu {
    ...
    public static String input(Scanner sc, String msg) {
        ...
        try {
            ...
            if (value.length() > maxLen || !value.matches("[-0-9]+"))
                throw new InputMismatchException();
        } catch (Exception e) {
            // sc.next(); //uses up invalid token to avoid infinite loop | DEPRECATED
            return input(sc,
                    "invalid input, please enter a number with maximum of 10 digits\n input: ");
        }
        // --------------------
    ...
    public static boolean validate(String accountNumberString) {
        StringBuilder sb = new StringBuilder(accountNumberString);
        accountNumberString = sb.reverse().toString();
